...
<form @submit.prevent="modifierproduit">
 ...
 <div class="form-group">
 <FilePond max-files="1" @change="onFileChange" :files="'storage/images/'+article.imageartpetitf" />
</div>
 <div class="form-group">
...
export default { 
 ...
 this.axios.put(`http://localhost:8000/api/articles/${this.$route.params.id}`,this.article) 
 .then(() => { 
 ...
 onFileChange(e) { 
 this.article.imageartpetitf = e.target.files[0].name; 
...
{ 
 ...
 getCategories(){ 
 this.axios.get('http://localhost:8000/api/categories').then(res => { this.Categories = res.data; }).catch(error => { 
 console.log(error) 
 }); 
 }, 
 getOneArticle(){ 
